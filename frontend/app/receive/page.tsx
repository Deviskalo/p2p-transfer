...
export default function ReceivePage() {
  ...
  /*************  ✨ Docs for connectWebSocket ⭐  *************/
  /**
   * Connects to the server and registers as a receiver with the given
   * `senderId`.
   *
   * If the connection is successful, it sets the `isConnected` state to
   * `true` and the `isConnecting` state to `false`. If there is an error,
   * it sets the `error` state to the error message.
   *
   * If the connection is closed, it sets the `isConnected` state to `false`
   * and the `isConnecting` state to `false`. If the maximum number of
   * reconnect attempts has not been reached, it schedules a reconnect after
   * a delay of `RECONNECT_DELAY` milliseconds.
   */
  const connectWebSocket = () => {
    ...
    ws.onmessage = (event) => {
      if (typeof event.data === "string") {
        try {
          ...
          console.error(`Error parsing message: ${e}`, true);
        }
    ...
    ws.onerror = (error) => {
      console.error(`WebSocket error: ${error}`, true);
    };
